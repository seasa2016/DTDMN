    ...
    def __init__(self, config, vocab=None, key_vocab=None, ignore_vocab=None):
        ...
        if vocab is not None:
            if(key_vocab is not None):
                print("Use extra cost for key words")
                self.weight = torch.ones(len(vocab))
                for _ in key_vocab:
                    self.weight[_] = 10.0

            if(ignore_vocab is not None):
                print("Use extra vocab for ignore words")
                self.ignore = torch.ones(len(vocab), dtype=torch.float)
                for _ in ignore_vocab:
                    self.ignore[_] = 0.0
