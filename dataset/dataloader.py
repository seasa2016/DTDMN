...
class persuasiveDataset(Dataset):
    def __init__(self, data_path, embed_path, pair_path, max_adu=128, top=3, train=False, direct=False):
        self.top = top
        self.train = train
...
if(__name__ == '__main__'):
    data_path = './../../preprocess/cmv_raw_origin/holdout/tree/train/data'
    embed_path = './../../preprocess/cmv_raw_origin/holdout/tree/cmv_elmo'
    pair_path = './../../preprocess/cmv_raw_origin/holdout/tree/train/graph_pair'
    ...
    for i, datas in enumerate(train_dataloader):
        ...
        for side, data in enumerate(datas):
            l = data['elmo_emb'].shape[-2]
            for j, para_span in enumerate(data['para_span']):
                for index, start, end in para_span:
                    if(end>=l):
                        print(i*8, side, j)
