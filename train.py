...
def update(outputs, criterion=None, dtype=None):
    ...
    if(dtype=='tree'):
        ...
        for output in outputs:
            temp = torch.cat(output, dim=-1).view(-1)
            stat.append( (criterion(temp, torch.ones_like(temp)), temp.mean(), (temp>0).float().mean()))
    return stat
...
def train(args, persuasive_data_iter, tree_data_iter, model, criterion, device, multitask=False):
    ...
    alpha = (args.ac_type_alpha, args.link_type_alpha)
    ...
    for count in range(1, total_epoch+1):
        ...
        for data in datas:
            ...
            pre_out, last_out = model(**data)
            outputs.append((pre_out, last_out))

        # simply compare two value
        pred = [(outputs[1][1]-outputs[0][1]).view(-1),
                (outputs[1][1]-outputs[1][0]).view(-1)]
        for i, _ in enumerate(pred):
            persuasive_preds[i].append(_.detach().cpu())

        loss = criterion(pred[0], torch.ones_like(pred[0]))
        #loss += criterion(pred[1], torch.ones_like(pred[1]))
        ...
        if(multitask and (count%tree_count==0)):
            ...
            loss, loss_stat, output = tree_cal_score(output, label, None, alpha)
            loss.backward()
        ...
        if(count % eval_every==0):
            stat = update(persuasive_preds, criterion, dtype='persuasive')

            nt = time.time()
            print('now:{}, time: {:.4f}s'.format(count, nt - t),
                '\npair: [loss: {:.4f}, diff: {:.4f}, acc: {:.4f}]'.format(stat[0][0], stat[0][1], stat[0][2]),
                '\tself: [loss: {:.4f}, diff: {:.4f}, acc: {:.4f}]'.format(stat[1][0], stat[1][1], stat[1][2]),
                flush=True)
...
def test(epoch, persuasive_iter, tree_iter, model, criterion, device, alpha=[0, 0]):
    model.eval()
    t = time.time()
    ...
    with torch.no_grad():
        for i, datas in enumerate(persuasive_iter):
            ...
            for j, _ in enumerate(pred):
                total_preds[j].append(_.detach().cpu())
...
def main():
    ...
    print('dev:')
    test('End', persuasive[1], tree[1], model, criterion, device, alpha= (args.ac_type_alpha, args.link_type_alpha))
    print('test:')
    test('End', persuasive[2], tree[2], model, criterion, device, alpha= (args.ac_type_alpha, args.link_type_alpha))
